...
export type QuarkExclusionRecipeDefinition<
  T extends RecipeDefinition = RecipeDefinition,
> = T &
  Readonly<{
    true_type: string;
    exclusions?: string[];
...
> extends Recipe<QuarkExclusionRecipeDefinition<TDefinition>> {
  constructor(
    definition: QuarkExclusionRecipeDefinition<TDefinition>,
    private readonly trueRecipe: TRecipe,
  ) {
    super({
      ...definition,
      ...trueRecipe.toJSON(),
  ...
  replaceIngredient(replace: Replacer<Ingredient>): Recipe {
    return new QuarkExclusionRecipe(
      this.definition,
      this.trueRecipe.replaceIngredient(replace) as TRecipe,
    );
  }
  ...
  replaceResult(replace: Replacer<Result>): Recipe {
    return new QuarkExclusionRecipe(
      this.definition,
      this.trueRecipe.replaceResult(replace) as TRecipe,
    );
  }
}
...
> {
  create(
    definition: QuarkExclusionRecipeDefinition,
