- [ ] seperate rules from modifier, turn into glorified predicate (or simply predicate)
- [ ] add rule service as parent of rule emitter, usuable for something like recipe graph
- [ ] write-able registry wrapper around loaded values, allowing emitters to update/add entries
- [ ] add abstract emitter combining ruled & custom
- [ ] add Predicates services for loot tables & recipes
- [x] fix JSON loader not working with subfolders (worldgen/structure) to create IDs, pass folder to them?
- [ ] predicates helpers (and, or, always, never), simplify existing rules
- [ ] expose API to register stub registry ID types & add all vanilla ones by default (depending on pack format)